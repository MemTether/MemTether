...
ASSETS = [
    ...
    ("Visual Studio Code", r"<PATH>\AppData\Local\Programs\Microsoft VS Code\Code.exe", "local_tool",
     "Code.exe",
     r'test -f "<PATH>/AppData/Local/Programs/Microsoft VS Code/Code.exe"',
     "代码编辑器,插件扩展",
     "★在 AppData\\Local\\Programs 下，不在 Program Files。"),
    ("Python 3.10(系统)", r"<PATH>\AppData\Local\Programs\Python\Python310\python.exe", "runtime",
     "python.exe",
     r'test -f "<PATH>/AppData/Local/Programs/Python/Python310/python.exe"',
     "Python 运行时(系统版)",
     "★优先用 WorkBuddy 托管的 3.13.12。"),
    ("Windows Terminal", r"<PATH>\AppData\Local\Microsoft\WindowsApps\wt.exe", "local_tool",
     "wt.exe",
     r'test -f "<PATH>/AppData/Local/Microsoft/WindowsApps/wt.exe"',
     "终端",
     ""),
    ("winget", r"<PATH>\AppData\Local\Microsoft\WindowsApps\winget.exe", "local_tool",
     "winget install/search/list",
     r'test -f "<PATH>/AppData/Local/Microsoft/WindowsApps/winget.exe"',
     "包管理器,装机/查已装软件",
     "★查「本机装了什么」可用 winget list，比扫目录权威。"),
    ("NanaZip", r"<PATH>\AppData\Local\Microsoft\WindowsApps\NanaZip.exe", "local_tool",
     "NanaZip.exe",
     r'test -f "<PATH>/AppData/Local/Microsoft/WindowsApps/NanaZip.exe"',
     "压缩解压(现代 7-Zip 分支)",
     ""),
    ("Bandizip", r"<PATH>\AppData\Local\Microsoft\WindowsApps\bandizip.exe", "local_tool",
     "bandizip.exe",
     r'test -f "<PATH>/AppData/Local/Microsoft/WindowsApps/bandizip.exe"',
     "压缩解压",
     ""),
    ...
    ("DrvCeo驱动总裁", r"C:\DrvCeonw\DrvCeo.exe", "local_tool",
     ...
     "驱动安装/更新",
     "★在 C:\\DrvCeonw\\（非标准目录）。"),
    ("Discord PTB", r"<PATH>\AppData\Local\Discord\app-1.0.9256\Discord.exe", "local_tool",
     "Discord.exe",
     r'test -f "<PATH>/AppData/Local/Discord/app-1.0.9256/Discord.exe"',
     "即时通信(公测版)",
     "★版本号在路径里，升级后会变，校验会失活属正常。"),
    ("YTDownloader", r"<PATH>\.ytDownloader\ytdlp.exe", "local_tool",
     "ytdlp.exe",
     r'test -f "<PATH>/.ytDownloader/ytdlp.exe"',
     "视频下载(yt-dlp 封装)",
     ""),
    ...
    ("Lenovo Service Bridge", r"<PATH>\AppData\Local\Programs\Lenovo\Lenovo Service Bridge\CreateWTSTask.exe", "local_tool",
     "CreateWTSTask.exe",
     r'test -f "<PATH>/AppData/Local/Programs/Lenovo/Lenovo Service Bridge/CreateWTSTask.exe"',
     "联想设备服务",
     ""),
    # ===== E 盘补录（第一轮漏掉的）=====
