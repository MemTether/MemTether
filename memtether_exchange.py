...
def _content_blake3(content):
    """Schema v2 P0: per-fact content hash (BLAKE3).
    Falls back to sha256[:64] when blake3 is not installed."""
    if _HAS_BLAKE3:
        return "b3:" + blake3.blake3(content.encode("utf-8")).hexdigest()
...
def export_exchange(db_path=None, out_path=None, include_retired=False,
    ...
    # Schema v2 P0: attach producer + per-fact content hash
    for f in facts:
        f["producer"] = producer
        f["content_blake3"] = _content_blake3(f["content"])
