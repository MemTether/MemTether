...
**已知短板（不藏）**：
- **注入槽位瓶颈（第一+二阶段已缓解）**：活跃条目数百条，但受客户端注入上限约束，每轮实际只能喂进几十条 → 存得多、喂得少。
  09-25 十一修：band 内已加 Q-Value 优先（高 Q 条目带内优先入槽），「用得多 → 留得住」链路已通；
...
- [x] 自动接入器覆盖 macOS / Linux 的客户端路径（E-1 已用 `_pick_config` 三平台路径；Windows 实测通过，macOS/Linux 路径待社区反馈）
