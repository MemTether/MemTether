...
**三条实测事实**（09-18 首测 79 个；09-25 复测 103 个）：

1. **装一处，两版生效** —— 分发走文件级指针，读写同一份物理文件，没有同步逻辑就不会漂移。
2. **治理杠杆不在"装几个"，在"每个描述多少字"** —— 09-25 复测：**103 个技能的注入成本是 37,266 字符**
   （name+description，平均 361/个；比 09-18 的 79 个 / 26,013 涨了 43%）。
   真正会挤爆上下文的是描述写得太长，而不是数量。
3. **解析器必须兼容 YAML 块标量** —— 很多 `SKILL.md` 用 `description: >` 把描述折成多行。
   单行正则只会读到那个 `>`：09-18 实测注入成本被低估 3,144 字符（13.7%），
...
顶层平铺 **83 个 `.py`**（下表覆盖 66 个核心模块，其余为小型辅助脚本）。平铺是为了「方式 B」下能 `python gateway.py …` 直接跑；
...
| 评测 / 回归 | 17 | 📊 想复跑卷子时用 | `asset_bench` `asset_bench_holdout` `asset_selfcheck` `bench_longmemeval` `e2e_verify` `hard_bench` `hard_holdout` `judge_selfcheck` `qvalue_ab` `qvalue_upshift_test` `refuse_bench` `regression_test` `rerank_k_bench` `smoke_bitemporal` `test_autosync` `test_triggers` `test_pii_roundtrip` |
...
**已知短板（不藏）**：
- **注入槽位瓶颈（第一阶段已缓解）**：活跃条目数百条，但受客户端注入上限约束，每轮实际只能喂进几十条 → 存得多、喂得少。
  09-25 十一修：band 内已加 Q-Value 优先（高 Q 条目带内优先入槽），「用得多 → 留得住」链路已通；但总预算 3980 字符的硬限未变，仍是长期瓶颈
...
- **Q-Value「机制三层已通、数据仍在积累」**：09-25 三条回写路径全部就位——
  ...
  ③ CLI `mem.py qvalue <uid>`。投影侧也已接通：band 内高 Q 优先入槽（十一修）。但"全自动判断这条记忆有没有被**采纳**"（而非仅命中）仍需客户端配合，仍是路线图项。
  另有 09-24 时间衰减（`q_decay = 0.5^(days/90)`，90 天半衰期）：旧条目"硬度"随时间自然下降，新条目获得相对优先级。
- **投影钉住（`pin`）只是缓解、不是解决**：它让少数"必须一直在"的定义类结论
  免于被时间序挤出；十一修后高 Q 条目也有了带内优先，但注入槽位的总量瓶颈没变。
...
- [x] 被采纳价值分（Q-Value）—— 检索命中被采纳后回写、下次上浮；**检索排序 + 投影 band 内优先均生效**
...
- [x] 注入槽位策略优化 · 第一阶段（09-25 十一修）— band 内 Q-Value 优先：rebuild 排序键从 `(band, lead长度, seq)` 改为 `(band, -q_value, lead长度, seq)`，高 Q 条目带内优先入槽；Q=0.5 退化短条优先零差异。借鉴 context-engine 槽位分级 + leanctx loss-tolerance routing。后续可做：类型感知失真容忍（pin 全量 / experience 更激进截断）
